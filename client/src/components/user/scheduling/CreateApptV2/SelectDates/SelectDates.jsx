...
export default class SelectDates extends React.Component{
    ...
    dateClicked = (key,firstSpot) =>{
        let newState = this.state;
        newState.isSelected = key;
        this.props.toggleTime(null,key,firstSpot);
    ...
    render(){
        ...
        const list = () =>{
            return this.props.data.map((item,key)=>{
                if(this.props.spots[key].length==0){
                    cardStyles.opacity = "0.5";
                }
                if(this.state.isSelected){
                    cardStyles.borderColor = '#CB0806';
                }
                //onClick={(e)=>{this.props.toggleTime(null,this.props.spots[key][0])}}
                return(
                        <div className="col-lg-2" key={key}>
                            <div className="card" className={"my-card "+(key==this.props.dateSelectedIndex? "my-card-selected ":"")+(this.props.spots[key].length==0? "my-card-disabled ":"")} onClick={()=>{this.dateClicked(key,this.props.spots[key][0])}}>
                                <div className="card-body">
                                    ...
                                    {this.props.spots[key].length>0?
                                        <>
                                            {/* <p className="text-center">{this.props.spots[key].length} availible times</p> */}
                                        </>
                                    :
                                    null
                                    }
